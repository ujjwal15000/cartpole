...
class Agent:
    def __init__(self, model=None, weights=None, render=False):
        self.k_crossover = 2  # num crossover points
        self.max_treward = 0
        self.env = gym.make("MountainCarContinuous-v0", render_mode="human") if render else gym.make("MountainCarContinuous-v0")
    ...
    def _create_model(weights=None):
        ...
        model.add_layer(32, 2, "ReLU")
        model.add_layer(64, activation_function="ReLU")
        model.add_layer(64, activation_function="ReLU")
        model.add_layer(1, activation_function="Sigmoid")
    ...
    def train(self, episodes):
        ...
        for e in range(1, episodes + 1):
            ...
            for f in range(1, 1000):
                ...
                action = self.model.predict(state.reshape(1, 2))[0]
                ...
                if done or trunc:
                    ...
                    break
        self.max_treward = np.mean(rewards) if len(rewards) > 0 else 0
    ...
    def test(self, episodes):
        ...
        for e in range(1, episodes + 1):
            ...
            for f in range(1, 1000):
                ...
                total_reward += reward
                if done or trunc:
                    self.max_treward = max(self.max_treward, total_reward)
...
class GeneticAlgo:
    ...
    def crossover(self, gen):
        ...
        if sorted_pop[0].max_treward > 40:
            rewards = Agent(sorted_pop[0].model, render=False).test(5)

            if np.mean(rewards) > 40:
                Agent(sorted_pop[0].model, render=True).test(1)
                time.sleep(2)
